...
| REG-014 | Portfolio Analysis | Generation Detail sheet (old-gen vs. continuing article/margin split) | **Confirmed** (methodology), figures **Provisional** per REG-012 | New Sheet 3 in `Project_Bob_Portfolio_Tiering_08092026.xlsx`, one to two rows per one of the 260 REG-010-touched franchises: an "Old-gen (Clearance)" row (its clearance-list SKUs) and, where one survives, a "Continuing (new-gen)" row (everything else), each with its own Article(s)/Model(s)/Sales/GM%. Lets a reader see e.g. whether a Problem Child franchise's margin gap is coming from the exiting old-gen SKU or the surviving one — example found: "Front Proof Jacket" (Men) old-gen 42.2% GM% vs. continuing 34.7% GM%, i.e. the *current* generation is the actual margin problem, not legacy stock. **Caveat:** Sales/GM% are recomputed directly from raw SKU-level exports (same Core-scope rule as REG-008), not the published Sheet-2 figures — the two generations' Sales for a franchise won't necessarily sum to the franchise's audited Sales_2025 (REG-012's unresolved ordertype gap). Read as relative/directional (which generation is bigger, which margins better), not as an audited SEK split. GM% blanked below 50,000 SEK/year per REG-004's precedent rather than shown as a wild percentage (164 of 472 rows affected). Also found: 210 of 1,845 distinct raw Article strings have some but not all of their color-SCOs on a clearance list ("mixed") — resolved by working at SCO grain throughout rather than by Article-name text, so this doesn't cause mis-splits. |

---

*Seeded 4 Sep 2026 from the SS26 DTC & Wholesale portfolio review. Reconciled against `config.py` 7 Sep 2026. Clearance treatment (REG-010/011/012), tier consolidation (REG-013), and Generation Detail sheet (REG-014) added 8 Sep 2026. Update the live artifact first, then re-sync this file.*
