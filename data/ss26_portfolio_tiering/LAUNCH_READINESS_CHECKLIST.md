...
- [x] **Confirm nothing load-bearing only exists in this chat transcript.** *Audited 8 Sep 2026. Found and fixed one gap: the "Wholesale expansion opportunity" glossary mapping existed only in Tier Bench's system prompt, not in the register — added to both. Code-level judgment calls (News package fallback list, stock-file column-index assumption, why scripts don't re-sort the sheet, the top-10 cap on stock detail tables) were already captured as docstrings/comments in `scripts/`, which is the right home for implementation detail rather than the register.*
  **Fully closed 9 Sep 2026.** Tier Bench's page generator (`gen_page.py`) is committed at `data/ss26_portfolio_tiering/tier_bench/`, along with the seven `qa_data/*.json` files it reads and a rebuild script for every one of them: `build_art_channel.py` + `build_qa_data.py` (franchises, tier_summary, channel_mix, generation_comparison) and `extract_stock.py` / `extract_country_breakdown.py` / `extract_regional_tiering.py` (stock, country-breakdown, regional-tiering). All 7 verified structurally equal, not just matching row counts, against the previously-published file before being trusted — this caught two real bugs along the way (`extract_country_breakdown.py`/`extract_regional_tiering.py` defaulting a genuinely-blank cell to `0` instead of `None`; `build_qa_data.py`'s franchise loader silently missing 7 fields that predate REG-015–021 being added to Sheet 2). Refreshing Tier Bench's dataset after a future data drop is now a documented command sequence (see `tier_bench/README.md`'s "Regenerating the full dataset"), the same standard as the Excel pipeline (Section D) — no longer a candidate item, just routine maintenance going forward.
